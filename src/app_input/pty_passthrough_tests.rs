...
use super::pty_passthrough::{
    AttachedTerminalProbe, attached_terminal_probe_from_lock, ctrl_c_passthrough_may_forward,
    probe_attached_terminal, CtxArc,
};
...
fn probe_attached_terminal_reports_busy_when_ctx_mutex_held() {
    ...
    let _guard = ctx.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    assert_eq!(
